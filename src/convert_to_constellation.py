...
def parse_args():
    """
    Parse command-line arguments for SNR, modulation types, limit, batch_size, image_types, and workers.
    """
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for modulation_type in mods_to_process:
            ...
            for snr_value in snrs_to_process:
                ...
                # Submit a separate task to the executor for each modulation/SNR combination
                futures.append(
                    executor.submit(process_modulation_snr_set, modulation_type, snr_value, h5_file_path, 32, image_types)
                )
