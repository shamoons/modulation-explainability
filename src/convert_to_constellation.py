...
def group_by_modulation_snr(dataloader: DataLoader, mod2int: Dict[str, int]) -> Tuple[Dict[str, Dict[float, List[torch.Tensor]]], Dict[int, str]]:
    """
    Group the dataset by modulation type and SNR, filtering only by specified SNRs and modulation types.
    """
...
def process_by_modulation_snr(grouped_data: Dict[str, Dict[float, List[torch.Tensor]]], output_dir: str = 'constellation', save_constellation: bool = True, save_gray: bool = True, save_enhanced_gray: bool = True, save_three_channel: bool = True) -> None:
    """
...
if __name__ == "__main__":
    ...
    snrs_to_process = [20, 30]  # Specify SNRs of interest or set to None for all
    mods_to_process = ['QPSK']  # Specify modulation types of interest or set to None for all

    # Get data loader and define the batch size and SNRs to process
    dataloader, mod2int = get_dataloader(batch_size=4096, snr_list=snrs_to_process, mods_to_process=mods_to_process)
    ...
    process_by_modulation_snr(grouped_data, output_dir='constellation', save_constellation=False, save_gray=False, save_enhanced_gray=False, save_three_channel=True)
